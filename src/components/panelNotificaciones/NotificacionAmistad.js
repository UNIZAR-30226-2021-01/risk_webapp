...
import { MDBRow, MDBCol, MDBBtn, MDBIcon, MDBModal, MDBModalHeader, MDBModalBody,
...
export const NotificacionAmistad = ({datos}) => {
	...
	const decisionAmistad = (datos, decision) => async() => {
		...
		if (data.code === 0) {
			console.log('Amigo con id y nombre: ' + datos.idEnvio + ' ' + datos.infoExtra + ' has elegido ' + decision)
		} else {
    ...
    return (
		<>
			<MDBRow>
				<MDBCol md="8">
					<p>Solicitud de amistad de <strong>{datos.infoExtra}</strong></p>
				</MDBCol>
				<MDBCol md="4">
					<MDBRow className="mr-1">
						<MDBCol md="6" >
							<MDBBtn color="success" className="btn-notis" 
								onClick={decisionAmistad(datos, "Aceptar")}>
								<MDBIcon icon="check" />
							</MDBBtn>
						</MDBCol>
						<MDBCol md="6">
							<MDBBtn color="danger" className="btn-notis" 
			...
			<MDBModal isOpen={isOpen} toggle={() => {toggle()}} >
				<MDBModalHeader toggle={() => {toggle()}}>Se ha producido un error:</MDBModalHeader>
				<MDBModalBody>
					<ErroresServer serverErrors={serverErrors} />
				</MDBModalBody>
				<MDBModalFooter className="d-flex justify-content-around">
					<MDBRow>
						<MDBBtn color="primary" className="btn-modal" onClick={() =>{toggle()}}> Aceptar</MDBBtn>
					</MDBRow>
				</MDBModalFooter>
			</MDBModal>
		</>
